...
class ACTEncoder(nn.Module):
    ...
    def forward(
        self, x: Tensor, pos_embed: Tensor | None = None,  dataset_index_embed: Tensor | None = None, key_padding_mask: Tensor | None = None
    ) -> Tensor:
        for layer in self.layers:
            x = layer(x, pos_embed=pos_embed, dataset_index_embed=dataset_index_embed, key_padding_mask=key_padding_mask)
        x = self.norm(x)
        return x
...
class ACTEncoderLayer(nn.Module):
    ...
    def forward(self, x, pos_embed: Tensor | None = None, dataset_index_embed: Tensor = None, key_padding_mask: Tensor | None = None) -> Tensor:

...
def main(hydra_cfg, root_dir, output_file):
    ...
    for subdir in os.listdir(root_dir):
        if subdir.isdigit():  # Check if the subdirectory name is numerical
            ...
            checkpoint_path = os.path.join(subdir_path, 'pretrained_model/model.safetensors')
            if os.path.isfile(checkpoint_path):
                policy = make_policy(hydra_cfg=hydra_cfg, pretrained_policy_name_or_path=subdir_path + "/pretrained_model")
                mse, l1 = evaluate_checkpoint(policy, validation_loader, device)
                results.append({'checkpoint': subdir_path, 'l1_loss': l1, 'step': int(subdir), 'mse_loss': mse})
...
if __name__ == "__main__":
    policy_path = Path('/Users/mbar/Desktop/projects/huggingface/lerobot/outputs/test_mps_operations/checkpoints/100100/pretrained_model')
    ...
    main(cfg, '/Users/mbar/Desktop/projects/huggingface/lerobot/outputs/test_mps_operations/checkpoints', 'dev/test.json')
